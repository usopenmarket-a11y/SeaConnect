...
// ── Availability calendar + Weather forecast ──────────────
function AvailabilityWeather({ boat, region }) {
  ...
  for (let i = 0; i < 42; i++) {
    ...
    const status =
      ...
      bk < 10 ? 'hold' :
      'open';
    const price = status === 'limited' ? boat.price + 380 : boat.price;
    days.push({ day, status, price });
  }

  const sel = days.find(d => d.day === selected) || {};

  // 7-day weather forecast (starts from selected day)
  const weatherBase = region === 'alex' ? { temp: 22, wind: 14, vibe: 'cool' } :
                       region === 'luxor' ? { temp: 32, wind: 8, vibe: 'hot' } :
                       { temp: 27, wind: 12, vibe: 'warm' };
  const icons = {
    sun: <SunIcon />,
  ...
  const forecast = Array.from({ length: 7 }, (_, i) => {
    ...
    const iconKey = precip > 15 ? 'cloud' : wind > 16 ? 'wind' : precip > 8 ? 'partial' : 'sun';
  ...
  return (
    <div className="avail-block">
      <div className="avail-head">
        <div>
          <div className="subhead" style={{ margin: 0, paddingBottom: 0, border: 0 }}>الإتاحة والطقس</div>
          <div style={{ fontFamily: 'var(--ff-mono)', fontSize: 10, letterSpacing: '0.12em', color: 'var(--muted)', textTransform: 'uppercase', marginTop: 4, direction: 'ltr' }}>
            AVAILABILITY & FORECAST · {boat.regionEn.toUpperCase()} · {boat.coords}
          </div>
        </div>
      ...
      <div className="avail-grid">
            ...
            {days.map((d, i) => {
              ...
              return (
                <button
                  ...
                  onClick={() => d.status !== 'booked' && setSelected(d.day)}
        ...
        <div className="weather-panel">
          {/* Selected day big weather */}
          <div className="weather-hero">
            <div>
              <div style={{ fontFamily: 'var(--ff-mono)', fontSize: 10, letterSpacing: '0.12em', color: 'var(--muted-2)', textTransform: 'uppercase', direction: 'ltr' }}>
                {monthsEn[month]} {selected}, 2026 · {boat.regionEn.toUpperCase()}
              </div>
              ...
              {(() => {
                ...
                return (
                  <>
                    ...
                    {pts.map(([x, y, t], i) => (
                      <g key={i}>
                        <circle cx={x} cy={y} r="3.5" fill="var(--foam)" stroke="oklch(0.58 0.14 50)" strokeWidth="2" />
                        <text x={x} y={y - 10} fill="var(--ink)" fontFamily="Geist Mono" fontSize="11" textAnchor="middle" fontWeight="500">{t}°</text>
                      </g>
                    ))}
                  </>
                );
          ...
          <div className="forecast-row">
            {forecast.map((f, i) => (
              <div key={i} className={`fc ${i === 0 ? 'active' : ''}`}>
                ...
                <div className="ico">{icons[f.iconKey]}</div>
                <div className="temps">
      ...
      {sel.status && sel.status !== 'booked' && (
        <div className="avail-footer">
          <div>
            <div style={{ fontFamily: 'var(--ff-mono)', fontSize: 10, letterSpacing: '0.12em', color: 'var(--muted)' }}>SELECTED · التاريخ المختار</div>
            <div style={{ fontFamily: 'var(--ff-display)', fontSize: 26, fontWeight: 700, marginTop: 4 }}>
              {selected} {months[month]} · {weekDays[0]}
            </div>
          </div>
          <div style={{ textAlign: 'left' }}>
            <div style={{ fontFamily: 'var(--ff-mono)', fontSize: 10, letterSpacing: '0.12em', color: 'var(--muted)' }}>PRICE · السعر</div>
            <div style={{ fontFamily: 'var(--ff-display)', fontSize: 26, fontWeight: 700, marginTop: 4, direction: 'ltr' }}>
              <span className="num">{(sel.price || boat.price).toLocaleString('en')}</span>
              <span style={{ fontFamily: 'var(--ff-mono)', fontSize: 12, color: 'var(--muted)', fontWeight: 400, marginRight: 6 }}>EGP</span>
            </div>
          </div>
          <div className={`avail-pill ${sel.status}`}>
            {sel.status === 'open' ? '✓ متاح للحجز' : sel.status === 'limited' ? '⚡ آخر فرصة · طلب عالٍ' : '⏱ معلّق · راجع الربان'}
          </div>
        </div>
      )}
    </div>
...
// ── Weather SVG icons (hand-drawn, nautical feel) ──────
function SunIcon() {
  return (
    <svg viewBox="0 0 40 40" fill="none">
      ...
      {Array.from({ length: 8 }).map((_, i) => {
        ...
        return <line key={i} x1={x1} y1={y1} x2={x2} y2={y2} stroke="oklch(0.58 0.14 50)" strokeWidth="1.5" strokeLinecap="round" />;
      })}
    </svg>
  );
}
function PartialCloudIcon() {
  return (
    <svg viewBox="0 0 40 40" fill="none">
      <circle cx="15" cy="16" r="6" fill="oklch(0.78 0.15 70)" stroke="oklch(0.58 0.14 50)" strokeWidth="1.5" />
      <path d="M 14 26 Q 10 26 10 22 Q 10 19 13 19 Q 14 15 18 15 Q 22 15 23 19 Q 28 19 28 24 Q 28 27 25 27 L 14 27 Z" fill="var(--foam)" stroke="var(--ink)" strokeWidth="1.5" strokeLinejoin="round" />
    </svg>
  );
}
function CloudIcon() {
  return (
    <svg viewBox="0 0 40 40" fill="none">
      <path d="M 10 26 Q 6 26 6 22 Q 6 18 10 18 Q 11 13 16 13 Q 22 13 24 18 Q 30 18 30 23 Q 30 27 26 27 L 10 27 Z" fill="var(--sand-2)" stroke="var(--ink)" strokeWidth="1.5" strokeLinejoin="round" />
    </svg>
  );
}
function WindIcon() {
  return (
    <svg viewBox="0 0 40 40" fill="none" stroke="var(--tide)" strokeWidth="1.8" strokeLinecap="round">
...
function WindMini() {
  return (
    <svg viewBox="0 0 14 14" width="12" height="12" fill="none" stroke="currentColor" strokeWidth="1.3" strokeLinecap="round" style={{ verticalAlign: 'middle', marginLeft: 3 }}>
...
Object.assign(window, { AvailabilityWeather });
